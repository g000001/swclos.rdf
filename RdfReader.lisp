...
;;;
;;; Copyright ｩ 2002,2004 by Galaxy Express Corporation, Japan.
...
(defun read-string (stream closech)
  #+excl (excl::read-string stream closech)
  #+sbcl (sb-impl::read-string stream closech))
  ...
  (let ((name (prop-name prop))
    ...
    (let ((resource (getf atts '|rdf|:|resource|))
      ...
      (cond (nodeID
            ...
            (resource
             ;(format t "~%*base*:~S~%*default-namespace*:~S" *base-uri* *default-namespace*)
  ...
  (setq *defined-resources*
        (mapcar #'(lambda (x) `(,x line nil))
          '(|rdfs|:|Resource| |rdfs|:|Literal| |rdf|:|Property| |rdfs|:|label| 
            |rdfs|:|comment| |rdfs|:|isDefinedBy| |rdfs|:|domain| 
            |rdfs|:|range| |rdfs|:|subClassOf| 
            |rdfs|:|subPropertyOf| |rdfs|:|seeAlso| |rdfs|:|isDefinedBy| 
            |rdfs|:|Class| |rdf|:|type| 
            |rdf|:|predicate| |rdf|:|subject| |rdf|:|object| |rdf|:|Statement| 
            |rdfs|:|Datatype| |rdf|:|XMLLiteral| 
            |rdf|:|List| |rdf|:|nil| |rdf|:|first| |rdf|:|rest| 
            |rdf|:|value| |xsd|:|anySimpleType| |xsd|:|boolean| 
            |xsd|:|anyURI| |xsd|:|string| |xsd|:|float| |xsd|:|double| 
            |xsd|:|unsignedByte| |xsd|:|unsignedShort|  
            |xsd|:|unsignedInt| |xsd|:|unsignedLong| |xsd|:|decimal| 
            |xsd|:|integer| |xsd|:|long| |xsd|:|int| 
            |xsd|:|short| |xsd|:|byte| |xsd|:|nonNegativeInteger| 
            |xsd|:|positiveInteger| 
            |xsd|:|nonPositiveInteger| |xsd|:|negativeInteger|
            |xsd|:|duration|
            |owl|:|DataRange| |owl|:|DeprecatedProperty| 
            |owl|:|DeprecatedClass| 
            |owl|:|incompatibleWith| 
            |owl|:|backwardCompatibleWith|
            |owl|:|priorVersion| 
            |owl|:|versionInfo|
            |owl|:|imports| |owl|:|OntologyProperty| |owl|:|Ontology| 
            ...
            |owl|:|minCardinality| |owl|:|someValuesFrom| |owl|:|hasValue| 
            |owl|:|allValuesFrom| |owl|:|onProperty| )))
  )

(defun find-external-format (code)
  #+excl (excl:find-external-format code)
  #+sbcl (sb-impl::find-external-format code))
...
(defun read-rdf-file (accepter-fun &optional (file (ask-user-rdf-file)) (code :default))
  ...
  (with-open-file (stream (pathname file) :external-format (find-external-format code))
    (let ((*line-number* 1)
          (*line-pos* 0)
          (*pos* 0)
