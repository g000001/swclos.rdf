...
(let ((xlist (cons (list #\null) nil)))
  ...
  (labels ((xnull () (char= #\null (caar xlist)))
    ...
    (defun putback-char (char stream)
      ...
      (xpush char)
      (decf *pos*)
      (when (char= char #\Newline) (decf *line-number*))
      char)
    ...
    (defun match-pattern-p (pattern stream)
      ...
      (loop for i :of-type fixnum from (length (car xlist)) to (length pattern)
          do (xput (read-char stream)))
      (loop for char1 :of-type character across pattern
    ...
    (defun skip-pattern (pattern stream)
      ...
      (loop for i :of-type fixnum from 1 to (length pattern) do (xpop)))
