...
(declaim ((or function symbol) *uri2symbol-name-mapping-fun*
    ...
    (cond (Prefix
          ...
          (*default-namespace*
           ...
           (cond (pkg (shadow LocalPart pkg)
                 ...
                 (t (setq QName LocalPart)
                    (warn "No package definition for default namespace ~S" *default-namespace*)
          ...
          (*base-uri*
           ...
           (cond (pkg (shadow LocalPart pkg)
                      ...
                      (export QName pkg))
                 (t (setq QName LocalPart)
                    (warn "No package definition for base uri ~S" *base-uri*)
                    (error "What shoud we do?"))))
...
(defun symbol2uri (symbol)
  ...
  (or (and (boundp symbol)
      ...
      (let* ((name (iri-escape (symbol-name symbol)))
